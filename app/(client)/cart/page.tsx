...
import useStore from '@/(client)/context/store';
  ...
  const handleCheckout = async () => {
    ...
    try {
      const metadata: Metadata = {
        ...
        customerName: user?.profile?.name ?? 'Unknown',
        customerEmail: user?.email ?? 'Unknown',
        address: selectedAddress,
      };
      const checkoutUrl = await createCheckoutSession(groupedItems, metadata);
      if (checkoutUrl) {
        window.location.href = checkoutUrl.success_url;
      }
    } catch (error) {
  ...
  return (
    <div className="bg-gray-50 pb-52 md:pb-10">
      {user ? (
        ...
        <Link
          href="/login"
          className="flex items-center justify-center h-screen text-2xl font-semibold"
        >
